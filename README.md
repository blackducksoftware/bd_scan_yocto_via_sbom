...
- Scan a Yocto project to create an SPDX SBOM file which is uploaded to the specified Black Duck server to create a project version
- Optionally filter recipes using data from the OpenEmbedded APIs to 'fix-up' recipes moved to new layers or with different versions/revisions
- Signature scan packages/downloaded archives
...
There are several important options to modify the behaviour including:
...
- Use data from the OpenEmbedded API to verify recipes, layers, version to ensure they match known components in the BD KB (use --get_oe_date)
- Cache OE data in JSON files in a local folder to remove need to download on every run (use --oe_data_folder)
- Specify semantic version distance for matching recipes against OE data (default distance is '0.0.0' - use --max_oe_version_distance)
- Create SPDX output file and do not upload to Black Duck to create project (use --output_file)
...
## USAGE
     ...
     --get_oe_data         Download and use OE data to check layers, versions &
                           revisions
     ...
     --scan_all_files      Signature scan all files (only recipes not matchedfrom
...
: Download layers/recipes/layers from layers.openembedded.org APIs to review origin layers and revisions used in recipes to 
...
: Do not send identified package and downloaded archives for Signature scanning. By default, if --get_oe_data is used, only recipes
not matched against OE data will be scanned, use --scan_all_files to scan all recipes.
